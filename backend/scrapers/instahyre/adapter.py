...
Primary: GET https://www.instahyre.com/jobseeker/api/jobs/?page={n}
Fallback: GET https://www.instahyre.com/api/v1/jobs/?format=json&page={n}

Indian market scraper — salary in INR/LPA, multi-city locations,
experience ranges in "X-Y years" format.
Uses curl_cffi with impersonate="chrome124" (same pattern as naukri/adapter.py).
"""
...
# curl_cffi headers matching a real Chrome browser
...
def _parse_salary_inr(item: dict) -> tuple:
    """
    Parse salary from Instahyre job.
    Returns (salary_min, salary_max, salary_display).
    """
    # Try salary_range dict
    ...
    if isinstance(salary_range, dict):
        ...
        if sal_min or sal_max:
            display = f"₹{sal_min or 0} - ₹{sal_max or sal_min} LPA"
    ...
    if isinstance(salary_range, str) and salary_range.strip():
        ...
        return sal_min, sal_max, salary_range.strip()
...
def _parse_experience(item: dict) -> tuple:
    """Parse experience from 'experience_range' or 'experience' fields."""
...
async def scrape_instahyre(
    max_pages: int = 5,
) -> List[Dict[str, Any]]:
    """
    Fetch jobs from Instahyre using curl_cffi with Chrome TLS impersonation.
    Falls back to alternate API endpoint on 403.
    """
    try:
        import curl_cffi.requests as cfr
    except ImportError:
        logger.error("curl_cffi not installed — cannot scrape Instahyre")
        return []

    session = cfr.AsyncSession(impersonate="chrome124", headers=_HEADERS, timeout=30)
    jobs: List[Dict[str, Any]] = []

    try:
        # Warm session
        try:
            await session.get("https://www.instahyre.com/")
        except Exception:
            pass
        ...
        for page in range(1, max_pages + 1):
            ...
            # If primary fails with 403, switch to fallback
            ...
            # Parse response — Instahyre may use "results", "data", or top-level list
            page_jobs = []
...
async def _fetch_page(session, url: str) -> Optional[dict]:
...
def _parse_job(item: dict) -> Optional[Dict[str, Any]]:
    """Parse a single Instahyre job item into our schema."""
    # Instahyre uses "designation" for title
    ...
    # ── Location (may be string or list) ─────────────────────
    ...
    # ── Salary (INR) ─────────────────────────────────────────
    salary_min, salary_max, salary_display = _parse_salary_inr(item)
...
if __name__ == "__main__":
    ...
    async def main():
        jobs = await scrape_instahyre(max_pages=1)
        print(f"Fetched {len(jobs)} jobs from Instahyre")
        for j in jobs[:3]:
            print(f"  {j['title']} @ {j['company']} — {j['location']} — {j.get('salary_display', '')}")
